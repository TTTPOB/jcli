...
from .common import Scope, force_option, validate_force


def _is_managed_shim(content: str) -> bool:
    """Recognize only shims generated by this setup command."""
    prefix = (
        "#!/usr/bin/env bash\nset -euo pipefail\nexec j-cli _hooks pre-commit-pair-sync"
    )
    if not content.startswith(prefix) or not content.endswith("\n"):
        return False
    args_text = content[len(prefix) : -1]
    try:
        args = shlex.split(args_text)
    except ValueError:
        return False
    if len(args) % 2 or args[::2] != ["--include"] * (len(args) // 2):
        return False
    return (
        content
        == prefix
        + "".join(f" --include {shlex.quote(value)}" for value in args[1::2])
        + "\n"
    )
...
def git_setup(
    ...
    if hook_path.exists() and not _is_managed_shim(
        hook_path.read_text(encoding="utf-8")
    ):
        if not force:
            emit_error(
                "HOOK_CONFLICT",
                f"{hook_path} is not a jcli-managed hook; use --force to replace it.",
                ctx.use_json,
            )
