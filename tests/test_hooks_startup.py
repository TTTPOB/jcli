...
def test_hooks_parser_import_does_not_load_tree_sitter():
    """Importing the hook parser itself (not calling its functions) must be free."""
    code = (
        "import jupyter_jcli.commands.hooks.parser;"
        " import sys;"
        " bad = [m for m in sys.modules if 'tree_sitter' in m];"
        " assert not bad, f'tree_sitter eagerly imported: {bad}'"
    )
    ...
    assert result.returncode == 0, (
        f"tree_sitter was eagerly imported by hook parser module-level code:\n"
        f"{result.stderr}"
    )
...
def test_tree_sitter_loaded_after_first_parse():
    ...
    code = (
        "from jupyter_jcli.commands.hooks.parser import iter_simple_commands;"
